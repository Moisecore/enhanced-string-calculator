...
let receiveInput =
    printfn @"Enter a string containing a delimiter character, and any number of positive integers separated by the delimiter, 
...
let getDelimiter (input:string) = 
    ...
    if partialStrInParts[0].Length < 3 then 
        ""
    else 
        partialStrInParts[0][2..]
