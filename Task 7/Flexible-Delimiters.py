...
def Add(strIn):
    ...
    # 4th: check for delimiter and set it from the input. Already supported longer delimiters, added brackets removal
    if(len(partialStrInParts[0]) < 5):
        print("ERROR: input string has no delimiter")
        return 0
    untrimmedDelimiter = partialStrInParts[0][2:]
    delimiter = untrimmedDelimiter.strip("[]")
...
inputString = input("""Enter a string containing a delimiter string enclosed by square brackets, and any number of positive integers 
