...
let receiveInput =
    printfn @"Enter a string containing a delimiter string enclosed by square brackets, and any number of positive integers 
    separated by the delimiter, following the format //[delimiter]\n[numbers...]
...
// Get the delimiter from the input string. Already supported longer delimiters, added brackets removal
let getDelimiter (input:string) = 
    let partialStrInParts = input.Split("\\n")
    if partialStrInParts[0].Length < 5 then 
        ""
    else 
        partialStrInParts[0][3..partialStrInParts[0].Length-2]

// Split the input string by the delimiter
let splitInput (input:Result<string,ErrorMessage>) =
    match input with
        | Ok strInput -> 
            let delimiter = getDelimiter strInput
            if delimiter = "" then
                Error NoDelimiterFound
            else
                let numbersPartStr = strInput.Split("\\n")[1]
                Ok (numbersPartStr.Split(delimiter, System.StringSplitOptions.RemoveEmptyEntries))
        | Error err -> Error err
