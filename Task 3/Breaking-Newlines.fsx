...
let receiveInput =
    printfn @"Enter any number of integers separated by a comma or a newline (\n):"
...
let splitInput (input:Result<string,ErrorMessage>) =
    let delimiters = [|",";"\\n"|] // Added an array of delimiters to split the string with
    match input with
        | Ok strInput -> Ok (strInput.Split(delimiters, System.StringSplitOptions.RemoveEmptyEntries))
        | Error err -> Error err
